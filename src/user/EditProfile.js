...
class EditProfile extends Component {
    ...
    init = (userId) => {
        ...
        .then(data => {
            if (data.error) {
                ...
                this.setState({id: data._id, name: data.name, email: data.email, error:'', bio:data.bio})
            }
        })
    }
    ...
    isValid = () => {
        ...
        if (fileSize > 1000000) {    //1mb file size
            this.setState({error: "File size exceeds 1MB!"})
            return false
        }
        if (name.length === 0) {
        ...
        if (password.length >=1 && !/\d/.test(password)) {
            this.setState({error: "Password must contain at least a digit!"})
            return false
        }
        return true 
    }
    ...
    signupForm = (name, email,password, bio) => (
        <form>
            ...
            <div className="form-group">
                ...
                <textarea onChange={this.handleChange("bio")} type="text" className="form-control" value={bio}/>
            </div>
            <div className="form-group">
    ...
    render() {
        const {id,name, email,password, redirectToProfile, error, loading, bio} = this.state
        ...
        return (
            <div className="container">
                ...
                {this.signupForm(name, email,password, bio)}            
            </div>
        );
    }
}
